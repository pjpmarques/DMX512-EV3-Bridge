...
DMX::DMX(int uart_number, int rx, int tx, int en) :
    uart(uart_number), rx_pin(rx), tx_pin(tx), en_pin(en) {
}
...
void DMX::set_timeout(unsigned milisecconds) {
  this->timeout = milisecconds;
}